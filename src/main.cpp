...
void loop() {
  ...
  float Suhu = sensors.getTempCByIndex(0); // Mendapatkan suhu pertama
  ...
  if (Firebase.ready() && signupOK && (millis() - sendDataPrevMillis > 15000 || sendDataPrevMillis == 0)) {
    ...
    // Write temperature
    if (Firebase.RTDB.setFloat(&fbdo, "/sensor/suhu_air", Suhu)) {
    ...
    // Write TDS
    if (Firebase.RTDB.setFloat(&fbdo, "/sensor/kekeruhan_air", tds)) {
    ...
    // Write timestamp
    if (Firebase.RTDB.setString(&fbdo, "/sensor/timestamp", currentTime)) {
